function [rt, choice, argmaxR, m_mr1c, m_mr2c, m_mr1cD, m_mr2cD, m_mg1c, m_mg2c, m_mg1cD, m_mg2cD, m_md1c, m_md2c, m_md1cD, m_md2cD] = LDDM_NClassDynmc_Trim_GPU(Vprior, Vinput, w, a, b,...
...
% G neurons
mg1c = gpuArray.nan([numel(dot_ax), sizeComput]); % mean rates sorted at beginning of the dot motion task
mg2c = gpuArray.nan([numel(dot_ax), sizeComput]);
mg1cD = gpuArray.nan([-min(sac_ax)+sum(sac_ax >= 0), sizeComput]); 
mg2cD = gpuArray.nan([-min(sac_ax)+sum(sac_ax >= 0), sizeComput]); % mean rates sorted at the time of decision
% D neurons
md1c = gpuArray.nan([numel(dot_ax), sizeComput]); % mean rates sorted at beginning of the dot motion task
md2c = gpuArray.nan([numel(dot_ax), sizeComput]);
md1cD = gpuArray.nan([-min(sac_ax)+sum(sac_ax >= 0), sizeComput]); 
md2cD = gpuArray.nan([-min(sac_ax)+sum(sac_ax >= 0), sizeComput]); % mean rates sorted at the time of decision
...
for ti = -pretask_steps:max(posttask_steps(:))
    ...
    if any(ti == dot_ax)
        ...
        mg1c(rec_axi,stllgo) = G1(stllgo);  % replace the running trials' values, keep others as nan
        mg2c(rec_axi,stllgo) = G2(stllgo);
        md1c(rec_axi,stllgo) = D1(stllgo);  % replace the running trials' values, keep others as nan
        md2c(rec_axi,stllgo) = D2(stllgo);
    end
    ...
    for excl = 1:numel(loc)
        ...
        mg1c(dot_ax > excldt,loc(excl)) = NaN;
        mg2c(dot_ax > excldt,loc(excl)) = NaN;
        md1c(dot_ax > excldt,loc(excl)) = NaN;
        md2c(dot_ax > excldt,loc(excl)) = NaN;
    end
    ...
    if ti > onset_of_stimuli + time_spcD % wait for recording until 200ms after stimuli on
        ...
        mg1cD(1:(-min(sac_ax)-1), Continue) = mg1cD(2:-min(sac_ax),Continue); % push the values into the queue
        mg1cD(-min(sac_ax),Continue) = G1(Continue); % update R1 from saved buffer
        mg2cD(1:(-min(sac_ax)-1), Continue) = mg2cD(2:-min(sac_ax),Continue); % push into the queue
        mg2cD(-min(sac_ax),Continue) = G2(Continue);
        md1cD(1:(-min(sac_ax)-1), Continue) = md1cD(2:-min(sac_ax),Continue); % push the values into the queue
        md1cD(-min(sac_ax),Continue) = D1(Continue); % update R1 from saved buffer
        md2cD(1:(-min(sac_ax)-1), Continue) = md2cD(2:-min(sac_ax),Continue); % push into the queue
        md2cD(-min(sac_ax),Continue) = D2(Continue);
        % for chains just made decision
        mr1cD(-min(sac_ax)+1,flip) = R1(flip);
        mr2cD(-min(sac_ax)+1,flip) = R2(flip);
        mg1cD(-min(sac_ax)+1,flip) = G1(flip);
        mg2cD(-min(sac_ax)+1,flip) = G2(flip);
        md1cD(-min(sac_ax)+1,flip) = D1(flip);
        md2cD(-min(sac_ax)+1,flip) = D2(flip);
        ...
        for si = 1:numel(tplist) % loop over different time stamps
            ...
            mg1cD(-min(sac_ax)+1+tplist(si), updatecells) = G1(updatecells);
            mg2cD(-min(sac_ax)+1+tplist(si), updatecells) = G2(updatecells);
            md1cD(-min(sac_ax)+1+tplist(si), updatecells) = D1(updatecells);
            md2cD(-min(sac_ax)+1+tplist(si), updatecells) = D2(updatecells);
        end
...
% G neurons, mean
m_mg1c = gpuArray.nan([numel(dot_ax), sizeComput(1), sizeComput(2)]); % average over all trials of the same condition
m_mg2c = gpuArray.nan([numel(dot_ax), sizeComput(1), sizeComput(2)]);
m_mg1cD = gpuArray.nan([numel(sac_ax), sizeComput(1), sizeComput(2)]);
m_mg2cD = gpuArray.nan([numel(sac_ax), sizeComput(1), sizeComput(2)]);
% D neurons, mean
m_md1c = gpuArray.nan([numel(dot_ax), sizeComput(1), sizeComput(2)]); % average over all trials of the same condition
m_md2c = gpuArray.nan([numel(dot_ax), sizeComput(1), sizeComput(2)]);
m_md1cD = gpuArray.nan([numel(sac_ax), sizeComput(1), sizeComput(2)]);
m_md2cD = gpuArray.nan([numel(sac_ax), sizeComput(1), sizeComput(2)]);
for i = 1:size(rt,1)
    for j = 1:size(rt,2)
        ...
        m_mg1c(:,i,j) = mean(mg1c(:,i,j,choose1),4,'omitnan');
        m_mg2c(:,i,j) = mean(mg2c(:,i,j,choose1),4,'omitnan');
        m_mg1cD(:,i,j) = mean(mg1cD([-min(sac_ax) + sac_ax(sac_ax < 0)' + 1, -min(sac_ax)+(1:sum(sac_ax>=0))],i,j,choose1),4,'omitnan');
        m_mg2cD(:,i,j) = mean(mg2cD([-min(sac_ax) + sac_ax(sac_ax < 0)' + 1, -min(sac_ax)+(1:sum(sac_ax>=0))],i,j,choose1),4,'omitnan');
        
        m_md1c(:,i,j) = mean(md1c(:,i,j,choose1),4,'omitnan');
        m_md2c(:,i,j) = mean(md2c(:,i,j,choose1),4,'omitnan');
        m_md1cD(:,i,j) = mean(md1cD([-min(sac_ax) + sac_ax(sac_ax < 0)' + 1, -min(sac_ax)+(1:sum(sac_ax>=0))],i,j,choose1),4,'omitnan');
        m_md2cD(:,i,j) = mean(md2cD([-min(sac_ax) + sac_ax(sac_ax < 0)' + 1, -min(sac_ax)+(1:sum(sac_ax>=0))],i,j,choose1),4,'omitnan');
        ...
        m_mg1c(dot_ax >= mRT + time_spc,i,j) = NaN;
        m_mg2c(dot_ax >= mRT + time_spc,i,j) = NaN;
        m_mg1cD(sac_ax <= -mRT + time_spcD,i,j) = NaN;
        m_mg2cD(sac_ax <= -mRT + time_spcD,i,j) = NaN;
        m_md1c(dot_ax >= mRT + time_spc,i,j) = NaN;
        m_md2c(dot_ax >= mRT + time_spc,i,j) = NaN;
        m_md1cD(sac_ax <= -mRT + time_spcD,i,j) = NaN;
        m_md2cD(sac_ax <= -mRT + time_spcD,i,j) = NaN;
    end
end
...
m_mg1c = gather(squeeze(m_mg1c));
m_mg2c = gather(squeeze(m_mg2c));
m_mg1cD = gather(squeeze(m_mg1cD));
m_mg2cD = gather(squeeze(m_mg2cD));
m_md1c = gather(squeeze(m_md1c));
m_md2c = gather(squeeze(m_md2c));
m_md1cD = gather(squeeze(m_md1cD));
m_md2cD = gather(squeeze(m_md2cD));
rt = gather(rt)*dt;
rt(rt == Inf) = nan;
