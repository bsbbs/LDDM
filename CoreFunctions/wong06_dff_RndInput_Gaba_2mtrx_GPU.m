function [rt, choice, argmaxR, dR] = wong06_dff_RndInput_Gaba_2mtrx_GPU(Vinput,Gaba,miu0,sgm,sgmInput,I0,JNL,wplus,...
...
m = size(miu0,2);
sizeComput = [n, m, sims];
JNs = gpuArray(repmat(wplus,1,1,sims));
miu0Array = gpuArray(repmat(miu0,1,1,sims));
JAextArray = gpuArray(JAext);
threshArray = gpuArray(thresh);
% cp1Array = gpuArray.ones(sizeComput)*V1mat/256;
% cp2Array = gpuArray.ones(sizeComput)*V2mat/256;
...
for kk = 1:stablizetime
    ...
    H1buffer(:,:,:,loci) = real(H1);
    H2buffer(:,:,:,loci) = real(H2);
end
...
for ti = 1:(onset_of_stimuli-1)
    ...
    x1 = JNs.*S1 + Gaba*JNL*S2 + I0 + Inoise1;
    x2 = Gaba*JNL*S1 + JNs.*S2 + I0 + Inoise2;
    H1old = H1;
    H2old = H2;
...
if numel(stimdur) == 1 % to cover the situation when onset_of_stimuli = offset_of_stimuli
    if stimdur < dt
        cp1Array = gpuArray(zeros(sizeComput));
        cp2Array = gpuArray(zeros(sizeComput));
    end
elseif prod(size(stimdur) == size(V1mat))
    stimdurmat = repmat(stimdur,1,1,sims);
    cp1Array(stimdurmat < dt) = 0;
    cp2Array(stimdurmat < dt) = 0;
...
I1 = JAextArray*miu0Array.*(cp1Array + gpuArray(randn(size(cp1Array))*sgmInput));
I2 = JAextArray*miu0Array.*(cp2Array + gpuArray(randn(size(cp2Array))*sgmInput));
...
for ti = (ti0 + 1):(max(total_time_steps(:)))
    ...
    x1 = JNs.*S1 + Gaba*JNL*S2 + I0 + I1 + Inoise1;
    x2 = Gaba*JNL*S1 + JNs.*S2 + I0 + I2 + Inoise2;
    H1old = H1;
    H2old = H2;
    H1 = (a*x1 - b)./(1 - exp(-d*(a*x1 - b)));
    H2 = (a*x2 - b)./(1 - exp(-d*(a*x2 - b)));
