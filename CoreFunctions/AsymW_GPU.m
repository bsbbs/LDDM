function [rt, choice, argmaxR] = AsymW_GPU(Vprior, Vinput, w, a, sgm, Tau, predur, dur, dt, presentt, triggert, thresh, initialvals, stimdur, stoprule, sims)
%%%%%%%%%%%%%%%%%%
...
for ti = -pretask_steps:max(posttask_steps(:))
    ...
    % update R, G
    G1old = G1; G2old = G2;
    G1 = G1 + (-G1 + w11p.*R1 + w12*R2)/Tau2*dtArray + InoiseG1;
    G2 = G2 + (-G2 + w21*R1 + w22p.*R2)/Tau2*dtArray + InoiseG2;
