function [R, G, I, rt, choice] = LcDsInhbt_dff_GABAblock(Vraw, GABA, w, a, b, sgm, Tau, dur, dt, presentt, triggert, thresh, initialvals, stimdur, stoprule)
tauN =0.002;
...
for ti = 2:(presentt/dt)
    ...
    dG = (-G(ti-1,:) + R(ti-1,:) * w - I(ti-1,:)*GABA) * dt/Tau(2);
    dR = (-R(ti-1,:) + 0./(1+G(ti-1,:)*GABA)) * dt/Tau(1);
    dInoiseG = (-InoiseG(ti-1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
    dInoiseI = (-InoiseI(ti-1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
    dInoiseR = (-InoiseR(ti-1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
...
for ti = (ti1st + 1):((triggert)/dt)
    ...
    dI = (-I(ti-1,:) + R(ti-1,:) * 0) * dt/Tau(3);
    dG = (-G(ti-1,:) + R(ti-1,:) * w - I(ti-1,:)*GABA) * dt/Tau(2);
    dR = (-R(ti-1,:) + (V + R(ti - 1,:)*a)./(1+G(ti-1,:)*GABA)) * dt/Tau(1);
    dInoiseG = (-InoiseG(ti-1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
    dInoiseI = (-InoiseI(ti-1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
    dInoiseR = (-InoiseR(ti-1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
    G(ti,:) = G(ti-1,:) + dG + InoiseG(ti-1,:);
...
while (tii < timeconstrain/dt) && dummy == 0
    ...
    tii = tii + 1;
    dI = (-I(ti + tii -1,:) + R(ti + tii -1,:) * b) * dt/Tau(3);
    dG = (-G(ti + tii -1,:) + R(ti + tii -1,:) * w - I(ti + tii -1,:)*GABA) * dt/Tau(2);
    dR = (-R(ti + tii -1,:) + (V + R(ti + tii - 1,:)*a)./(1+G(ti + tii -1,:)*GABA)) * dt/Tau(1);
    dInoiseG = (-InoiseG(ti + tii -1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
    dInoiseI = (-InoiseI(ti + tii -1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
    dInoiseR = (-InoiseR(ti + tii -1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
    ...
    if abs(R(ti + tii,1)-R(ti + tii,2)) >= thresh
        dummy = 1;
        rt = tii*dt;
...
elseif stoprule == 0 && tii < timeconstrain/dt
    ...
    while (tii < timeconstrain/dt)
        tii = tii + 1;
        dI = (-I(ti + tii -1,:) + R(ti + tii -1,:) * b*0) * dt/Tau(3);
        dG = (-G(ti + tii -1,:) + R(ti + tii -1,:) * w - I(ti + tii -1,:)*GABA) * dt/Tau(2);
        dR = (-R(ti + tii -1,:) + (V + R(ti + tii - 1,:)*a*0)./(1+G(ti + tii -1,:)*GABA)) * dt/Tau(1);
        dInoiseG = (-InoiseG(ti + tii -1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
        dInoiseI = (-InoiseI(ti + tii -1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
        dInoiseR = (-InoiseR(ti + tii -1,:).*dt./tauN + randn(sizeVraw).*sqrt(dt).*sgm.*dt./tauN);
        G(ti + tii,:) = G(ti + tii -1,:) + dG + InoiseG(ti + tii -1,:);
