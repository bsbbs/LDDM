function [rt, choice, argmaxR] = wong06_GPU(Vinput,miu0,sgm,I0,JN,...
...
for kk = 1:stablizetime
    ...
    H1 = (a*(x1 + Inoise1) - b)./(1 - exp(-d*(a*(x1 + Inoise1) - b)));
    H2 = (a*(x2 + Inoise2) - b)./(1 - exp(-d*(a*(x2 + Inoise2) - b)));
    inside = H1 >= 0;
    H1 = H1 .* inside;
    inside = H2 >= 0;
    H2 = H2 .* inside;
...
for ti = 1:(onset_of_stimuli-1)
    ...
    x1 = JN11*S1 + JN12*S2 + I0 + Inoise1;
    x2 = JN21*S1 + JN22*S2 + I0 + Inoise2;
    H1old = H1;
    H2old = H2;
...
for ti = (ti0 + 1):(max(total_time_steps(:)))
    ...
    x2 = JN21*S1 + JN22*S2 + I0 + I2 + Inoise2;
    H1old = H1;
    H2old = H2;
    H1 = (a*x1 - b)./(1 - exp(-d*(a*x1 - b)));
    H2 = (a*x2 - b)./(1 - exp(-d*(a*x2 - b)));
