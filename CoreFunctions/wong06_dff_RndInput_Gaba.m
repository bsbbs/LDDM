function [nu_wind, s_wind, rt, choice, H, S, Icourse] = wong06_dff_RndInput_Gaba(Vinput,Gaba, miu0,sgm,sgmInput,I0,JN,...
...
for kk = 1:stablizetime
    ...
    H = real((a*(xi+Inoise) - b)./(1 - exp(-d*(a*(xi+Inoise) - b))));
    H(H < 0) = 0;
...
for ti = 2:total_time_steps
    ...
    if ti >= onset_of_stimuli && isnan(rt)
        ...
        if abs(postsynapFR(1) - postsynapFR(2)) >= thresh
