function [R, G, I, rt, choice] = LcDsInhbtFD(Vinput, w, a, b1, b2, sgm, Tau, dur, dt, presentt, triggert1, triggert2, thresh, initialvals, stimdur, stoprule)
%%%%%%%%%%%%%%%%%%
...
for ti = 2:total_time_steps
    ...
    dI = (-I(ti-1,:)' + (b1*(ti >= onset_of_trigger1 & ti < onset_of_trigger2) + b2*(ti >= onset_of_trigger2))*R(ti-1,:)')/Tau(3)*dt;
    ...
    R(ti,R(ti,:) < 0) = 0;
    % threshold detecting
    if ti > onset_of_trigger2 && isnan(rt)
        if max(R(ti,:)) >= thresh
            rt = (ti-onset_of_trigger2)*dt;
