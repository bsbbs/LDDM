function [rt, choice, argmaxR, dR] = LcDsInhbt_RndInput_Gaba_2mtrx_GPU(Vinput, Gaba, w, a, b, sgm, sgmInput, Tau, dur, dt, presentt, triggert, thresh, initialvals, stimdur, stoprule, sims)
%%%%%%%%%%%%%%%%%%
...
beta11 = gpuArray(repmat(b,1,1,sims));
alpha22 = gpuArray(repmat(a,1,1,sims));
beta22 = gpuArray(repmat(b,1,1,sims));
% alpha12 = gpuArray.zeros(sizeComput);
% alpha21 = gpuArray.zeros(sizeComput);
% beta12 = gpuArray.zeros(sizeComput);
% beta21 = gpuArray.zeros(sizeComput);
X = ones(sizeComput);
...
V1Input = gpuArray.ones(sizeComput)*V1mat;
V2Input = gpuArray.ones(sizeComput)*V2mat;
...
for ti = (ti0 + 1):max(total_time_steps(:)) %(onset_of_trigger -1)
    ...
    G1 = G1 + (-G1 + w11*R1 + w12*R2 - Gaba*I1)/Tau2*dtArray + InoiseG1;
    G2 = G2 + (-G2 + w21*R1 + w22*R2  - Gaba*I2)/Tau2*dtArray + InoiseG2;
    I1 = I1 + (-I1 + beta11.*R1.*Continue.*BetasUp)/Tau3*dtArray + InoiseI1;
    I2 = I2 + (-I2 + beta22.*R2.*Continue.*BetasUp)/Tau3*dtArray + InoiseI2;
    ...
    R2 = R2 .* inside;
    % threshold detecting
    inside = ((R1 >= threshArray) + (R2 >= threshArray));% .* (abs(R1 - R2) >= threshArray/3);
