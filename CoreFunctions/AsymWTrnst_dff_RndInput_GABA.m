function [R, G, I, Vcourse, rt, choice] = AsymWTrnst_dff_RndInput_GABA(Vinput, Gaba, wraw, a, sgm, sgmInput, Tau, dur, dt, presentt, triggert, thresh, initialvals, stimdur, stoprule)
%%%%%%%%%%%%%%%%%%
...
%     dI = (-I(ti-1,:)' + b*(ti >= onset_of_trigger)*R(ti-1,:)')/Tau(3)*dt;
    dR = (-R(ti-1,:)' + (Vinput'*(ti >= onset_of_stimuli & ti < onset_of_stimuli+stim_duration) + a*(ti >= onset_of_stimuli)*R(ti-1,:)')./(1+G(ti-1,:)'*Gaba))/Tau(1)*dt;
