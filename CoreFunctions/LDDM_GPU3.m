function [choice, rt] = LDDM_GPU3(Vinput, w, a, b, sgm, Tau, dur, dt, presentt, triggert, thresh, initialvals, stimdur, stoprule, sims)
%%%%%%%%%%%%%%%%%%
...
%       - example      Vinput =  [320, 192, 192
%                                 288, 224, 224
%                                 270, 240, 240];
%    Format B: input values as a structure with the fields defining a V1,
%    V2, and V3 matrices, respectively. The dimensions of matrices need to
%    be the same as 2. Values at the same position
%       of the matrices will be used as a set of inputs.
%       - example      [V1, ~] =  meshgrid([320,288,270],[192,224,240])
%                      [V2 V3] =  meshgrid([192,224,240],[192,224,240])
%                      Vinput.V1 = V1; Vinput.V2 = V2; Vinput.V3 = V3;
...
R1new = gpuArray(X*initialvals(1,1)) + InoiseR1;
R2new = gpuArray(X*initialvals(1,2)) + InoiseR2;
R3new = gpuArray(X*initialvals(1,3)) + InoiseR3;
G1 = gpuArray(X*initialvals(2,1)) + InoiseG1;
G2 = gpuArray(X*initialvals(2,2)) + InoiseG2;
G3 = gpuArray(X*initialvals(2,3)) + InoiseG3;
I1 = gpuArray(X*initialvals(3,1)) + InoiseI1;
I2 = gpuArray(X*initialvals(3,2)) + InoiseI2;
I3 = gpuArray(X*initialvals(3,3)) + InoiseI3;
%% simulation
