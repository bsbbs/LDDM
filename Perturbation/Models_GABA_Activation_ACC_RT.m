...
mygray =  gray(2+length(cohr));
GABALevel = 10.^[linspace(0,.6021,21)];%1:10;
filename = sprintf('AsymW_GABA%i_%.1f_%1.1fa%1.0fw%1.1f_%1.1fsgm%1.1f_sgmInput%1.1f_values%i_sims%i',...
    length(GABALevel),min(GABALevel),max(GABALevel),a(1,1),w(1,1),w(1,2),sgm,sgmInput,numel(cohr),sims);
output = fullfile('./SimRslts',[filename, '.mat']);
if ~exist(output,'file')
    RT = [];
    Choice = [];
    for vi = 1:length(cohr)
        ...
        parfor gi = 1:length(GABALevel)
            Gaba = GABALevel(gi);
            [rt, choice, ~, ~] = AsymW_RndInput_GABA_GPU(Vinput, Gaba,...
                w, a, sgm, sgmInput, Tau, dur, dt, presentt, triggert, thresh, initialvals, stimdur, stoprule, sims);
            rt = gather(squeeze(rt));
            choice = gather(squeeze(choice));
            if sum(isnan(choice)) > sims/1.5
                choice = nan(size(choice));
            end
            if sum(isnan(rt)) > sims/1.5
                rt = nan(size(rt));
            end
            RT(:,gi,vi) = rt;
            Choice(:,gi,vi) = choice;
...
meanACC = squeeze(mean(2-Choice,1,'omitnan'));
h = figure;
figname = 'AsymW_ACC_RT_GABA_Value';
subplot(2,1,1); hold on;
for vi = 1:length(cohr)
    plot(GABALevel,meanRT(:,vi),'.-','MarkerSize',mksz-4,'LineWidth',lwd,'Color',mygray(vi,:));
end
...
meanACC = squeeze(mean(2-Choice,1,'omitnan'));
h = figure;
figname = 'AsymW_ACC_RT_Value_GABA';
subplot(2,1,1); hold on;
for gi = 1:length(GABALevel)
    plot(c,meanRT(:,gi),'-','MarkerSize',mksz-4,'LineWidth',lwd);
end
