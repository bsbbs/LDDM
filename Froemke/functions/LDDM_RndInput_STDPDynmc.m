...
for ti = (-pretask_steps):posttask_steps % align the beginning of the task as ti = 0
    ...
    if ischar(STDP_V)
        wV(ti+t_stamp+1,:) = wV(ti+t_stamp,:) + (-wV(ti+t_stamp,:)/tauS + gamma*(1-wV(ti+t_stamp,:)).*Vcourse(ti+t_stamp,:).*R(ti+t_stamp,:))*dt;
    elseif isnumeric(STDP_V)
        wV(ti+t_stamp+1,:) = STDP_V;
    end
    if ischar(STDP_a)
        wa(ti+t_stamp+1,:) = wa(ti+t_stamp,:) + (-wa(ti+t_stamp,:)/tauS + gamma*(1-wa(ti+t_stamp,:)).*R(ti+t_stamp,:).*(a*R(ti+t_stamp,:)')')*dt;
    elseif isnumeric(STDP_a)
        wa(ti+t_stamp+1,:) = STDP_a;
    end
    if ischar(STDP_G)
        wG(ti+t_stamp+1,:) = wG(ti+t_stamp,:) + (-wG(ti+t_stamp,:)/tauS + gamma*(1-wG(ti+t_stamp,:)).*G(ti+t_stamp,:).*R(ti+t_stamp,:))*dt;
    elseif isnumeric(STDP_G)
        wG(ti+t_stamp+1,:) = STDP_G;
    end
    if ischar(STDP_D)
        wD(ti+t_stamp+1,:) = wD(ti+t_stamp,:) + (-wD(ti+t_stamp,:)/tauS + gamma*(1-wD(ti+t_stamp,:)).*D(ti+t_stamp,:).*G(ti+t_stamp,:))*dt;
    elseif isnumeric(STDP_D)
