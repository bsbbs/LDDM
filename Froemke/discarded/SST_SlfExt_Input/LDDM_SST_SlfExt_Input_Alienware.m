...
addpath(fullfile(Homedir,'Documents','LDDM','Froemke/SST_SlfExt_Input/'));
cd('G:\My Drive\LDDM\Froemke\SST_SlfExt_Input');
...
b0 = 2.8;
task = 'RT';
%RT, stimdur = Inf, triggert = presentt + dt;
...
if ~exist(output,'file')
        ...
        Vinput.V1 = V1.*iSTDP; %scale*[1+cp, 1-cp];%*boost(level);
        Vinput.V2 = V2.*iSTDP;
        Vprior.V1 = ones(size(V1))*scale;
        Vprior.V2 = ones(size(V2))*scale;
        [rt, choice, ~] = LDDM_STDP_SlfExt_GPU(Vprior, Vinput, iSTDP, w, a, b,...
