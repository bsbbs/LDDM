function [R, G, I, Vcourse, rt, choice] = LcDsInhbt_RndInput_Gaba(Vinput, Gaba, w, a, b, sgm, sgmInput, Tau, dur, dt, presentt, triggert, thresh, initialvals, stimdur, stoprule)
%%%%%%%%%%%%%%%%%%
...
choice = NaN;
Vinputraw = Vinput;
Vinput = Vinputraw + randn(size(Vinput))*sgmInput*256/30;
Vcourse = Vinput;
%% simulation begin
G(1,:) = initialvals(2,:);
R(1,:) = initialvals(1,:);
...
for ti = 2:total_time_steps
    ...
    Vcourse(ti,:) = Vinput;
    % update R, G, I
    dG = (-G(ti-1,:)' + w * R(ti-1,:)' - Gaba*I(ti-1,:)')/Tau(2)*dt;
    dI = (-I(ti-1,:)' + b*(ti >= onset_of_trigger)*R(ti-1,:)')/Tau(3)*dt;
    dR = (-R(ti-1,:)' + (Vinput'*(ti >= onset_of_stimuli & ti < onset_of_stimuli+stim_duration) + a*(ti >= onset_of_stimuli)*R(ti-1,:)')./(1+Gaba*G(ti-1,:)'))/Tau(1)*dt;
