function [R, G, I, rt, choice] = LcDsInhbtStochasticInput(Vinput, w, a, b, sgm, Tau, dur, dt, presentt, triggert, thresh, initialvals, stimdur, stoprule)
%%%%%%%%%%%%%%%%%%
...
for ti = 2:total_time_steps
    % update Vinput
    dVnoise = (-(Vnoise(ti-1,:) - Vinput) + randn(sizeVinput).*sqrt(dt).*sgmInput)/tauInput*dt;
    Vnoise(ti,:) = Vnoise(ti-1,:) + dVnoise;
    % update R, G, I
    dG = (-G(ti-1,:)' + w * R(ti-1,:)' - I(ti-1,:)')/Tau(2)*dt;
    dI = (-I(ti-1,:)' + b*(ti >= onset_of_trigger)*R(ti-1,:)')/Tau(3)*dt;
    dR = (-R(ti-1,:)' + (Vnoise(ti,:)'*(ti >= onset_of_stimuli & ti < onset_of_stimuli+stim_duration) + a*(ti >= onset_of_stimuli)*R(ti-1,:)')./(1+G(ti-1,:)'))/Tau(1)*dt;
