function [choice, rt, R, G, I, Vcourse] = LDDM_STDP_av_RndInput(Vprior, Vinput, iSTDP, w, a, b,...
...
for ti = (-pretask_steps):posttask_steps % align the beginning of the task as ti = 0
    ...
    % update R, G, I
    dR = (-R(ti+t_stamp,:)' + (iSTDP.*V' + iSTDP.*a*R(ti+t_stamp,:)')./(1+iSTDP*G(ti+t_stamp,:)'))/Tau(1)*dt;
