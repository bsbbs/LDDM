function [choice, rt, R, G, I, Vcourse] = LDDM_RndInput_STDP(Vprior, Vinput, STDP_v, STDP_a, STDP_G, w, a, b,...
...
for ti = (-pretask_steps):posttask_steps % align the beginning of the task as ti = 0
    ...
    % update R, G, I
    dR = (-R(ti+t_stamp,:)' + (STDP_v.*V' + STDP_a.*a*R(ti+t_stamp,:)')./(1+STDP_G.*G(ti+t_stamp,:)'))/Tau(1)*dt;
