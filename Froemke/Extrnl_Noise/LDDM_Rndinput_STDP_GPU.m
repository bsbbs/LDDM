function [rt, choice, argmaxR, p1, p2] = LDDM_Rndinput_STDP_GPU(Vprior, Vinput, STDP_v, STDP_a, STDP_G, w, a, b,...
...
if all(size(STDP_a)==sizeVinput)
    STDP_a = gpuArray(repmat(STDP_a,1,1,sims));
end
if all(size(STDP_G)==sizeVinput)
    STDP_G = gpuArray(repmat(STDP_G,1,1,sims));
end
...
for ti = -pretask_steps:max(posttask_steps(:))
    ...
    R1 = R1 + (-R1 + ((V1Array + V1prArray*(ti<0)).*STDP_v + alpha11*R1.*STDP_a).*Continue./(1 + G1old.*STDP_G))/Tau1*dtArray + InoiseR1;
    R2 = R2 + (-R2 + ((V2Array + V2prArray*(ti<0)).*STDP_v + alpha22*R2.*STDP_a).*Continue./(1 + G2old.*STDP_G))/Tau1*dtArray + InoiseR2;
    % update noise
    InoiseR1 = InoiseR1 + (-InoiseR1 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
    InoiseR2 = InoiseR2 + (-InoiseR2 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
    InoiseG1 = InoiseG1 + (-InoiseG1 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
