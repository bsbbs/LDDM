function [rt, choice, argmaxR] = LDDM_Rndinput_STDP_GPU(Vprior, Vinput, w, STDP_v, STDP_a, STDP_G, a, b,...
...
for ti = -pretask_steps:max(posttask_steps(:))
    ...
    R1 = R1 + (-R1 + (STDP_v.*(V1Array + V1prArray*(ti<0)) + STDP_a.*alpha11*R1+STDP_a.*alpha12*R2).*Continue./(1+STDP_G.*G1old))/Tau1*dtArray + InoiseR1;
    R2 = R2 + (-R2 + (STDP_v.*(V2Array + V2prArray*(ti<0)) + STDP_a.*alpha21*R1+STDP_a.*alpha22*R2).*Continue./(1+STDP_G.*G2old))/Tau1*dtArray + InoiseR2;
    % update noise
    InoiseR1 = InoiseR1 + (-InoiseR1 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
    InoiseR2 = InoiseR2 + (-InoiseR2 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
    InoiseG1 = InoiseG1 + (-InoiseG1 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
