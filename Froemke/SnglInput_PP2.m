...
if show
    ...
    xlim([-Ntwk.Scale/2, Ntwk.Scale/2]);
    ylim([-Ntwk.Scale/4, Ntwk.Scale/4]);
    mysavefig(h, filename, outdir, 14, [3, 3], 2);
end
...
if Ntwk.show
    ...
    xlabel('Exct channels');
    ylabel('Exct channels');
    mysavefig(h, filename, outdir, 14, [3.2, 3.6]); % [4, 3]
    ...
    xlabel('Inhbt channels');
    ylabel('Exct channels');
    mysavefig(h, filename, outdir, 14, [1.6, 3]);
    ...
    xlabel('Exct channels');
    ylabel('Inhbt channels');
    mysavefig(h, filename, outdir, 14, [3.2, 1.46]);
    ...
    ylabel('Inhbt channels');
    xlabel('Time (secs)');
    mysavefig(h, filename, outdir, 14, [3, 3.5]);
    ...
    hold on;
    segment = round(smpl_time/dt);
    plot(smpl_time, G(IdxI(1),segment)/max(G(IdxI(1),segment)), '-', 'Color', "#659FBE", 'LineWidth',.5);
    plot(smpl_time, R(IdxE(1),segment)/max(R(IdxE(1),segment)), 'k-', 'LineWidth',.5);
    xlabel('Time (secs)');
    ylabel('Normalized activity');
    %ylabel('Firing rates (Hz)');
    mysavefig(h, filename, outdir, 14, [1.8, 1.2], 2);

    h = figure;
    filename = sprintf('RGExmplpair_at_E%iandI%i_%1.0fmins',IdxE(1), IdxI(1), max(time));
    hold on;
    lg2 = plot(time, G(IdxI(1),1:numel(time)), '-', 'Color', "#659FBE", 'LineWidth', .5);
    lg1 = plot(time, R(IdxE(1),1:numel(time)), 'k-', 'LineWidth', .5);
    plot([min(smpl_time), max(smpl_time), max(smpl_time), min(smpl_time), min(smpl_time)]/60, [5.7, 5.7, .1, .1, 5.7], '-k');
    xlim([0, 35]);
    ylim([0, 12]);
    ylabel('Firing rates (Hz)');
    xlabel('Time (mins)');
    ...
    plot([min(smpl_time), max(smpl_time), max(smpl_time), min(smpl_time), min(smpl_time)]/60, [.063, .063, .051, .051, .063]+.012, '-k');
    xlim([0, 35]);
    xlabel('Time (mins)');
    ylabel('Synaptic weight');
    mysavefig(h, filename, outdir, 12, [3, 2], 2);
