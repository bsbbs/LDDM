function [choice, rt, R, G, D, Vcourse] = LDDM_RndInputrv1(Vprior, Vinput, BR, BG, eltp, iltp, balance_scale, w, a, b,...
...
for ti = (-pretask_steps):posttask_steps % align the beginning of the task as ti = 0
    ...
    if ti > -pretask_steps && ti < 0
        V = Vprior*balance_scale;
    elseif ti >= onset_of_stimuli && ti < offset_of_stimuli
        if (mod(ti*dt, .005) == 0)
            Vnoise = randn(sizeVinput)*sgmInput;
        end
        V = Vinput*balance_scale + Vnoise;
    else
        V = zeros(sizeVinput);
    end
    ...
    % update R, G, I
    BD = BG;
    dR = (-R(ti+t_stamp,:)' + (V' + a*R(ti+t_stamp,:)' + BR)./(1+iltp*G(ti+t_stamp,:)'))/Tau(1)*dt;
    dG = (-G(ti+t_stamp,:)' + eltp*w * R(ti+t_stamp,:)'  + BG - D(ti+t_stamp,:)')/Tau(2)*dt;
    dD = (-D(ti+t_stamp,:)' + eltp*b*(ti >= onset_of_trigger)*R(ti+t_stamp,:)')/Tau(3)*dt;
