...
colorpalette = {'#ff0000','#0000ff','#06d6a0','#118ab2','#073b4c'};
...
dur = 60; % second
h = figure;
filename = sprintf('%s_BeforetoAfter_PV',task);
...
for testi = 1:2
    ...
    for i = [1,2]
        ...
        hst1 = histogram(R(5000:end,1),...
            'EdgeColor', 'none', 'FaceColor', mygray(1,:), 'FaceAlpha', .3, 'Normalization', 'pdf');
        pd1 = fitdist(R(:,1),'kernel','Kernel','normal');
        x = hst1.BinEdges;
        y1 = pdf(pd1,x);
        mylgd(i) = plot(x,y1, '-', 'Color', mycols(i,:), 'LineWidth', lwd);
        ylim([0, .18]);
        xlim([-5, 180]);
    end
    ylabel('Density');
    ...
    title(lgd, 'LTP');
    title(names(testi));
    mysavefig(h, filename, plotdir, fontsize, [6,2.2]);
end
...
task = sprintf('CovtCourse_N%i_SST',N);
predur = 0;
presentt = 0;
stimdur = Inf;
triggert = Inf;
...
for j = 1:2
    ...
    ldg = [];
    % yyaxis left;
    ldg(1) = plot(x1*dt,SNR(1:length(x1),2), '-', 'Color', mycols(j,:), 'LineWidth',lwd);
    plot(50+(x2-min(x2)+max(x1))*dt,SNR(length(x1)+1:end,2), '-', 'Color', mycols(j,:), 'LineWidth',lwd);
    ...
    mysavefig(h, filename, plotdir, fontsize, [3, 2]); % [2.5,1.57]
    % exportgraphics(h, fullfile(plotdir, [filename, '.pdf']), 'ContentType','vector');
end
