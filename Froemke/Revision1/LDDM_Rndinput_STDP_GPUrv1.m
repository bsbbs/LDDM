function [rt, choice, argmaxR, p1, p2] = LDDM_Rndinput_STDP_GPUrv1(Vprior, Vinput, STDP_V, STDP_RG, STDP_GR, w, a, b,...
...
if all(size(STDP_V)==sizeVinput)
    STDP_V = gpuArray(repmat(STDP_V,1,1,sims));
end
if all(size(STDP_RG)==sizeVinput)
    STDP_RG = gpuArray(repmat(STDP_RG,1,1,sims));
end
...
for ti = -pretask_steps:max(posttask_steps(:))
    ...
    G1old = G1;
    G2old = G2;
    G1 = G1 + (-G1 + w11*R1.*STDP_RG + w12*R2.*STDP_RG - I1)/Tau2*dtArray + InoiseG1;
    G2 = G2 + (-G2 + w21*R1.*STDP_RG + w22*R2.*STDP_RG  - I2)/Tau2*dtArray + InoiseG2;
    ...
    R1 = R1 + (-R1 + ((V1Array + V1prArray*(ti<0)).*STDP_V + alpha11*R1.*0).*Continue./(1 + G1old.*STDP_GR))/Tau1*dtArray + InoiseR1;
    R2 = R2 + (-R2 + ((V2Array + V2prArray*(ti<0)).*STDP_V + alpha22*R2.*0).*Continue./(1 + G2old.*STDP_GR))/Tau1*dtArray + InoiseR2;
    % update noise
    InoiseR1 = InoiseR1 + (-InoiseR1 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
    InoiseR2 = InoiseR2 + (-InoiseR2 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
    InoiseG1 = InoiseG1 + (-InoiseG1 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
    ...
    R2 = R2 .* inside;
    % Pre-Post STDP
    p1 = p1 + G1old.*R1.*Continue*dt;
    p2 = p2 + G2old.*R2.*Continue*dt;
    % threshold detecting
