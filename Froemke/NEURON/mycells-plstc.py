...
GABAA_TAU = 2 * ms # the decay time constant of IPSC, like GABAa
...
class Belt:
    ...
    def _create_cells(self, NE, NI, SPACE_EXTENT_X, SPACE_EXTENT_Y):
        ...
        for i in range(NE):
            self.Ecells.append(
                ENeuron(i, (NE-i)/NE * SPACE_EXTENT_X, random.uniform(-0.5, 0.5) * SPACE_EXTENT_Y, 0, n.PI/2)
            )
        self.Icells = []
        for i in range(NI):
            self.Icells.append(
                INeuron(i, (NI-i)/NI * SPACE_EXTENT_X, random.uniform(-0.5, 0.5) * SPACE_EXTENT_Y, 0, n.PI/2)
            )
    ...
    def _connect_EtoI(self):
        for source in self.Ecells:
            for target in self.Icells:
                dist = ((source.x - target.x)**2 + (source.y - target.y)**2 + (source.z - target.z)**2)**0.5
                conn_prob = 1 #CnnctProb_EtoI*np.exp(-.5*(dist/AxonRange_EtoI)**2)
                if random.random() < conn_prob:
    ...
    def _connect_ItoE_SST(self):
        for source in self.Icells:
            for target in self.Ecells:
                dist = ((source.x - target.x)**2 + (source.y - target.y)**2 + (source.z - target.z)**2)**0.5
                conn_prob = 1 #CnnctProb_ItoE*np.exp(-.5*(dist/AxonRange_ItoE)**2)
                if random.random() < conn_prob:
