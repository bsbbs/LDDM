...
mysavefig(h, filename, outdir, 12, [3.7, 3.3]); % [4, 3]
...
EEdynamc = Ntwk.wEE(98,32);
EIdynamc = Ntwk.wEI(32,98);
IEdynamc = Ntwk.wIE(98,32);
...
for ti = 1:(length(time)-1)
    ...
    preAP = R(:,ti)';
    postAP = R(:,ti);
    a_pre_EE = a_pre_EE + (-a_pre_EE/Ntwk.tau_prepost)*dt + Ntwk.A*preAP*dt; % the intermiediate variables for pre
    a_post_EE = a_post_EE + (-a_post_EE/Ntwk.tau_postpre)*dt + Ntwk.A*postAP*dt; % and post action potential induced plasticity
    a = postAP*dt*a_pre_EE - a_post_EE*preAP*dt; % the AP induced overall plasticity change
    ds = (-Ntwk.wEE/Ntwk.taus)*dt + Ntwk.gamma*(1 - Ntwk.wEE).*a.*Ntwk.Cnnct_EE; % a already contains the dt information, only applied to physical connections
    Ntwk.wEE = Ntwk.wEE + ds;
    Ntwk.wEE(Ntwk.wEE<0) = 0;
    EEdynamc(ti+1) = Ntwk.wEE(98,32);
    % apply E to I STDP kernel
    preAP = R(:,ti)';
    postAP = G(:,ti);
    a_pre_EI = a_pre_EI + (-a_pre_EI/Ntwk.tau_prepost)*dt + Ntwk.A*preAP*dt; % the intermiediate variables for pre
    a_post_EI = a_post_EI + (-a_post_EI/Ntwk.tau_postpre)*dt + Ntwk.A*postAP*dt; % and post action potential induced plasticity
    a = postAP*dt*a_pre_EI - a_post_EI*preAP*dt; % the AP induced overall plasticity change
    ds = (-Ntwk.wEI/Ntwk.taus)*dt + Ntwk.gamma*(1 - Ntwk.wEI).*a.*Ntwk.Cnnct_EI; % a already contains the dt information, only applied to physical connections
    Ntwk.wEI = Ntwk.wEI + ds;
    Ntwk.wEI(Ntwk.wEI<0) = 0;
    EIdynamc(ti+1) = Ntwk.wEI(32,98);
    ...
    ds = (-Ntwk.wIE/Ntwk.taus)*dt + Ntwk.gamma*(1 - Ntwk.wIE).*a.*Ntwk.Cnnct_IE; % a already contains the dt information, only applied to physical connections
    Ntwk.wIE = Ntwk.wIE + ds;
    Ntwk.wIE(Ntwk.wIE<0) = 0;
    IEdynamc(ti+1) = Ntwk.wIE(98,32);
end
fprintf('\n');
save(fullfile(outdir, 'Simulation_full.mat'), 'Ntwk', 'wEE_initial', 'wEI_initial', 'wIE_initial', 'EEdynamic', 'IEdynamc', 'Seq');
...
if Ntwk.show
    ...
    plot([min(smpl_time), max(smpl_time), max(smpl_time), min(smpl_time), min(smpl_time)]/60, [3.4, 3.4, 3.0, 3.0, 3.4]/1000, '-k');
    xlim([0, 35]);
    xlabel('Time (mins)');
    ylabel('Synaptic weight');
    mysavefig(h, filename, outdir, 12, [3, 2], 2);
