...
%% setup
outdir = '/Users/bs3667/Library/CloudStorage/GoogleDrive-bs3667@nyu.edu/My Drive/LDDM/Froemke/DynamicVersionSngl';
Setup;
...
mysavefig(h, filename, outdir, 12, [5, 2]);
...
rng(2023);
s = .01*randn(Ntwk.Exct.N, Ntwk.Inhbt.N).*Ntwk.Cnnct_IE; % synaptic weight from I to E, weak initial connections from the nearby SST 
s(s<0) = 0;
wIE_initial = s;
if Ntwk.show
    % synaptic connection weights
    h = figure;
    filename = sprintf('SynapticW_at_time%1.0fs', 0*dt);
    imagesc(wIE_initial');
    caxis([-.1, .1]);
...
smpl_time = [0:40000]*dt; % unit in Secs
smpl_time = [10000:30000]*dt; % unit in Secs
smpl_time = 5*60+[10000:30000]*dt; % unit in Secs
%% 
outdir = fullfile(outdir, 'PP');
if ~exist(outdir,'dir')
    mkdir(outdir);
end
...
if Ntwk.show
    ...
    plot([min(smpl_time), max(smpl_time), max(smpl_time), min(smpl_time), min(smpl_time)]/60, [9, 9, .3, .3, 9], '-k');
    xlim([0, 35]);
    ylabel('Firing rates (Hz)');
    xlabel('Time (mins)');
    ...
    filename = 's_dynamic';
    plot(time, sdynamc, 'k-', 'LineWidth',1);
    plot([min(smpl_time), max(smpl_time), max(smpl_time), min(smpl_time), min(smpl_time)]/60, [5.4, 5.4, 4.7, 4.7, 5.4]/100, '-k');
    xlim([0, 35]);
    xlabel('Time (mins)');
    ylabel('Synaptic weight');
    mysavefig(h, filename, outdir, 12, [3, 2], 2);
