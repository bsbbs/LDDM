...
for ii = 1:Ntwk.Input.N
    plot(smpl_time, Seq(1:numel(smpl_time),ii)+1.5*(ii-1), '-', "Color", OKeeffe(ii,:), 'LineWidth',2);
end
...
hold on;
for ii = 1:Ntwk.Input.N
    plot(time, Seq(1:numel(time),ii)+1.5*(ii-1), '-', "Color", OKeeffe(ii,:), 'LineWidth',1);
end
ylim([-.5, Ntwk.Input.N*1.5]);
plot([0, 0, max(smpl_time)/60, max(smpl_time)/60, 0], [-.5, Ntwk.Input.N*1.5, Ntwk.Input.N*1.5, -.5, -.5], 'r-');
yticks(1.5*([1:Ntwk.Input.N]-1));
yticklabels(num2str([1:Ntwk.Input.N]'));
ylabel('Input');
xlabel('Time (mins)');
...
if Ntwk.show
    ...
    filename = 's_dynamic';
    plot(time, sdynamc, 'k-', 'LineWidth',1);
    plot([min(smpl_time), max(smpl_time), max(smpl_time), min(smpl_time), min(smpl_time)]/60, [4.2, 4.2, 3.0, 3.0, 4.2]/1000, '-k');
    xlim([0, 35]);
    xlabel('Time (mins)');
    ylabel('Synaptic weight');
    mysavefig(h, filename, outdir, 12, [3, 2], 2);
