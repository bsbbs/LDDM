...
lgd6 = plot(D(:,2),'g--', 'Color','#06d6a0', 'LineWidth',lwd);
lgd5 = plot(D(:,1),'g-', 'Color','#06d6a0', 'LineWidth',lwd/2); % old color 669900
lgd2 = plot(R(:,2), 'k--', 'LineWidth',lwd);
lgd1 = plot(R(:,1), 'k-', 'LineWidth',lwd);
plot([predur + triggert + rt/2, predur + triggert + rt]/dtsavefigs,  [thresh, thresh],'k-','LineWidth',.5);
ylim([-5,max(ylim)]);
yticks([0, 70]);
...
mysavefig(h, filename, plotdir, fontsize, [3, 2.2]);