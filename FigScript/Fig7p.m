%% Fig 7 - Persistent activity with and without local disinhibition
...
for bi = 1:3
    ...
    lg1 = plot(R1Line,R2Line,'-','Color',colorpalette{4},'LineWidth',lwd/2);% dR1/dt = 0
    if w(2,2) - b > 0 && w(1,2) == w(1,1)
    ...
    lg2 = plot(R1Line,R2Line,'--','Color',colorpalette{1},'LineWidth',lwd/2);% dR2/dt = 0
    syms R1 R2;
    vars = [R1 R2];
...
%             plot([0, (alpha(1,1)-1)/(w(1,1) - b)],[(alpha(2,2)-1)/(w(2,2) - b),(alpha(2,2)-1)/(w(2,2) - b)],'k--','LineWidth',1); % constraints
            ...
            xticks([0, (alpha(1,1)-1)/(w(1,1)+w(1,2)-b), (alpha(1,1)-1)/(w(1,1) - b)]);
            yticks([0, (alpha(2,2)-1)/(w(2,2)+w(2,1)-b), (alpha(2,2)-1)/(w(2,2) - b)]);
        ...
        elseif b == 0
            ...
            xticks([0, (alpha(1,1)-1)/(w(1,1) - b)]);
            yticks([0, (alpha(2,2)-1)/(w(2,2) - b)]);
            xticklabels({'0','$\frac{\alpha-1}{\omega}$'});
            yticklabels({'0','$\frac{\alpha-1}{\omega}$'});
        elseif b >= 1
            xlim([-1.4,axismax]);ylim([-1.4,axismax]);
            xticks([0, (alpha(1,1)-1)/(w(1,1) + w(1,2) - b)]);
            yticks([0, (alpha(2,2)-1)/(w(2,2) + w(2,1) - b)]);
...
%     xticklabels({});
%     savefig(h, filename, outdir, fontsize, aspect2);
%     subplot(2,1,2);
    ylim([-.4,70]);
    yticks([0]);
    xlim([1, dur/dt]);
    ...
    savefigs(h, filename, plotdir, fontsize - 2, aspect10);
end
%% coding two items
mygrayred = mygray;
mygrayred(:,1) = 1;
...
%         lgd2 = plot(R(:,2), 'k--', 'Color', mygrayblue(vi+1,:), 'LineWidth',lwd/2);
    ...
    si = 1;
    h = figure;
    filename = sprintf('Fig7p_CodingFiveItems_VR_b%i_s%i',bi,si);
...
%     end
    h = figure;
    filename = sprintf('Fig7p_CodingFiveItems_WM_b%i_s%i',bi,si);
