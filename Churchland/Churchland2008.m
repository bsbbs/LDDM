...
% plot(ones(1,2)*min(x)+(delay+predur)/dt, [0,80],'k--');
% plot(ones(1,2)*190+predur/dt, [0,80],'k--');
% plot(ones(1,2)*max(x)+(delay+predur)/dt, [0,80],'k--');
% patch([min(x)+(delay+predur)/dt ones(1,2)*max(x)+(delay+predur)/dt min(x)+(delay+predur)/dt], [0, 0, 80, 80], [0.8 0.8 0.8], 'FaceAlpha',.3);
ylabel('Activity (H.z.)');
xlabel('Time (ms)');
xlim([-200,2000]);
xticks([0,1000,1190,1320]);
xticklabels({'','','',''});
