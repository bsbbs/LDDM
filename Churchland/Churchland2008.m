outdir = '/Volumes/GoogleDrive/My Drive/LDDM/Churchland2008';
addpath('/Users/bs3667/Documents/LDDM/CoreFunctions');
%% multi-alternative choice - dynamic no noise
paramspecify_LcDsInhbt;
% alpha beta2, sgm (unused),  scale (unused), beta4
params = [14	1.53	30	6	2.4];
% a, b, noise, scale, b2
Tau = [.1, .1, .1];
dt = .001;
sigma = 0;
...
for i = 1:2
    ...
    if items == 4
        beta = params(5) * eye(items);
    end
    sgm = params(3);
    w = 1*ones(items);
    initialvals = 10*[ones(1,items); items*ones(1,items); zeros(1, items)];
    Rstar = 40;
    ...
    stoprule = 1;
    [R0, G0, I0, ~, ~] = LcDsInhbt(Vprior, w, alpha, beta, sigma, Tau, predur+delay, dt, presentt, Inf, Inf, initialvals, stimdur+delay, stoprule);
    ...
    for cohr = [0, 9, 25.6]/100
        ...
        Rout = [R0;R];
        if items == 2
            lp(items) = plot(Rout(:,1),'k-','LineWidth',2.5);
        else
            lp(items) = plot(Rout(:,1),'r-','LineWidth',2.5);
        end
        plot(Rout(:,2),'--','Color',lp(items).Color,'LineWidth',2.5);
        if items == 4
...
xlabel('Time (a.u.)');
xlim([-100,2400]);
%xticks([round(presentt/dt),3000]);
%xticklabels({'cue','stimuli'});
...
saveas(h,fullfile(outdir,sprintf('Churchland_4items_sgm%1.1f.eps',sigma)),'epsc2');

%% multi-alternative choice, with noise
sims = 10000;
Cohr = [0 32 64 128 256 512 768]/1000; % percent of coherence
name = sprintf('sims%ik_a%1.2f_b%1.2f_noise%2.2f_scale%2.2f_b%1.2f',sims/1000,params);
...
if exist(file,'file')
    ...
    rtmat = [];
    choicemat = [];
    Buildup = NaN(131,sims,7,2);
    Suppress = NaN(131,sims,7,2);
    for i = 1:2
        items = i*2;
        ...
        for cohi = 1:length(Cohr)
            ...
            for rep = 1:sims
                ...
                if rt/dt >= 190+60-delay/dt
                    endt = min(320-delay/dt, rt/dt);
                    Buildup(1:(endt-190+delay/dt+1),rep,cohi,i) = R((190-delay/dt):endt,1);
                    Suppress(1:(endt-190+delay/dt+1),rep,cohi,i) = R((190-delay/dt):endt,2);
                end
            end
        end
        fprintf('\n');
    ...
    save(fullfile(outdir,sprintf('MultiChoice_%s.mat',name)),...
        'rtmat','choicemat','Buildup','Suppress','params','rndseed');
end
...
for i = 1:2
    ...
    for cohi = 1:length(Cohr)
        ...
        block_in = Buildup(:,in&longt,cohi,i);
        ...
        block_out = Suppress(:,out&longt,cohi,i);
...
if exist(file,'file')
    ...
    for i = 1:2
        ...
        w = 1*ones(items);
        initialvals = 8*[ones(1,items); items*ones(1,items); zeros(1, items)];
        Vprior = ((1-alpha(1,1))*Rstar + 2*Rstar^2)*ones(1,items);
        %Vprior = params(4)*512*ones(1,items);
        predur = .6;
        presentt = dt;
    ...
    save(fullfile(datadir,sprintf('MultiChoiceTrace_%s.mat',name)),...
        'rtmat','choicemat','R1mat','R2mat','params','rndseed');
end
%%
fontsize = 14;
h = figure; hold on;
