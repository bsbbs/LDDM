...
triggert1 = presentt;
triggert2 = 2.8;
stimdur = dur; %triggert2-presentt;
thresh = 70;
stoprule = 1;
c0 = [1,1,1];
...
for ei = 1:numel(eqlbvec) % the same equilibirum level and starting points but different alpha level
    ...
    for ai = 1:alen
        ...
        for vi = 1:numel(c3)
            ...
            initialvals = initialset*eqlbvec(ei);
            [R, G, I, rt, choice] = LcDsInhbtUrgencyGate(Vinput, w, a, b1, b2,...
                sgm, Tau, dur, dt, presentt, triggert1, triggert2, thresh, initialvals, stimdur, stoprule);
            R = [R0; R];
            lgd3(vi) = plot(R(:,1), R(:,2), '-', 'Color', mygray(vi+1,:), 'LineWidth',lwd/2);
            plot([0,R(round((predur+triggert1 + 2.8)/dt),1)],[0,R(round((predur+triggert1 + 2.8)/dt),2)],'.-', 'Color', mycol2(vi,:), 'LineWidth',lwd/2,'MarkerSize',mksz*2);
