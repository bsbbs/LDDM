...
for bi = 1:3
    ...
    lg1 = plot(R1Line,R2Line,'-','Color',colorpalette{4},'LineWidth',lwd);% dR1/dt = 0
    if w(2,2) - b > 0 && w(1,2) == w(1,1)
    ...
    lg2 = plot(R1Line,R2Line,'--','Color',colorpalette{1},'LineWidth',lwd);% dR2/dt = 0
    syms R1 R2;
    vars = [R1 R2];
...
%             plot([0, (alpha(1,1)-1)/(w(1,1) - b)],[(alpha(2,2)-1)/(w(2,2) - b),(alpha(2,2)-1)/(w(2,2) - b)],'k--','LineWidth',1); % constraints
    ...
    savefig(h, filename, outdir, fontsize, ratio);
end
