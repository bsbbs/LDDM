...
out_dir = './Rslts/FitLouie2011/R0GAMMA';
...
%%
function R1 = SolveEqlb(a,GAMMA,R0,V)
w = 1;
v = 1;
b = 0;
syms R1 R2
if V(1) > 0 && V(2) > 0
    eqns = [(V(1)/(R1-R0) - (w - b)*R1 - (GAMMA-a*R1/(R1-R0)))/v == R2, ... % dR1/dt = 0
        (V(2)/(R2-R0) - (w - b)*R2 - (GAMMA-a*R2/(R2-R0)))/v == R1];% dR2/dt = 0
    vars = [R1 R2];
    [AnswR1, ~] = solve(eqns, vars);
elseif V(1) > 0 && V(2) <= 0
    eqns = [(V(1)/(R1-R0) - (w - b)*R1 - (GAMMA-a*R1/(R1-R0)))/v == 0]; % dR1/dt = 0
    vars = [R1];
    AnswR1 = solve(eqns, vars);
elseif V(1) <= 0
    AnswR1 = 0;
