...
addpath('./SvrCode/');
out_dir = '../../RecurrentModel/Fit/Rslts/WW06FitBhvr7ParamsIV_QMLE_GPU';
...
%    JNp, JNn, I0, noise, miu0, gamma, tauNMDA
LB = [0    0     0      0       0  0 .001];
UB = [1    1     1      .1      120 2 1];
PLB = [.04  .04	.2      .01     20 .3 .01];
PUB = [.3   .3	.6      .04     60 .9 .2];
...
% likelihood function
% parpool(6);
nLLfun = @(params) WW06FitBhvr7ParamsIV_QMLE_GPU(params, dataBhvr);
[fvalbest,~,~] = nLLfun(x0)
...
out_dir = './Rslts/WW06FitBhvr7ParamsIV_QMLE_GPU';
if ~exist(out_dir,'dir')
    mkdir(out_dir);
end
...
if  ~exist(fullfile(plot_dir,sprintf('PlotData_%s.mat',name)),'file')
    tic;
    [nLL, Chi2, BIC, AIC, rtmat, choicemat] = WW06FitBhvr6ParamsIII_QMLE_GPU(params, dataBhvr);
    num2str(nLL)
    num2str(AIC)
    num2str(BIC)
...
if ~exist(fullfile(plot_dir,sprintf('PlotDynamic_%s.mat',name)),'file')
    tic;
    [nLL, Chi2, BIC, AIC, rtmat, choicemat,sm_mr1c, sm_mr2c, sm_mr1cD, sm_mr2cD] = WW06Dynamic_FitBhvr7ParamsIV_QMLE_GPU(params, dataDynmc, dataBhvr);
