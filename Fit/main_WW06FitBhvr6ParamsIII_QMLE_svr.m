...
addpath('./SvrCode/');
out_dir = '../../RecurrentModel/Fit/Rslts/WW06FitBhvr6ParamsIII_QMLE_GPU';
...
%    JNp, JNn, I0, noise, miu0, tauNMDA
LB = [0    0     0      0       0   .001];
UB = [1    1     1      .1      120  1];
PLB = [.04  .04	.2      .01     20  .01];
PUB = [.3   .3	.6      .04     60  .2];
...
% likelihood function
% parpool(6);
nLLfun = @(params) WW06FitBhvr6ParamsIII_QMLE_GPU(params, dataBhvr);
[fvalbest,~,~] = nLLfun(x0)
...
out_dir = './Rslts/WW06FitBhvr6ParamsIII_QMLE_GPU';
if ~exist(out_dir,'dir')
    mkdir(out_dir);
end
...
if ~exist(fullfile(plot_dir,sprintf('PlotDynamic_%s.mat',name)),'file')
    tic;
    [nLL, Chi2, BIC, AIC, rtmat, choicemat,sm_mr1c, sm_mr2c, sm_mr1cD, sm_mr2cD] = WW06Dynamic_FitBhvr6ParamsIII_QMLE_GPU(params, dataDynmc, dataBhvr);
