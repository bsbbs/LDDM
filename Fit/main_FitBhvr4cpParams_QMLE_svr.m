...
addpath('./SvrCode/');
out_dir = './Rslts/FitBhvr4cpParams_QMLE_SvrGPU';
...
%     a,    b, noise, scale, the coherence setting
LB = [0    0.1   .1    .1*256 [-1,-1,-1,-1,-1,-1]];
UB = [70   2	100  20*256 [1,1,1,1,1,1]];
PLB = [15  .7	5    1*256 [0,.032,.064,.128,.256,.512]-.3];
PUB = [60   1.2	40   8*256, [0,.032,.064,.128,.256,.512]+.3];
...
% likelihood function
% parpool(6);
nLLfun = @(params) LcDFitBhvr4cpParams_QMLE_GPU(params, dataDynmc, dataBhvr);
[fvalbest,~,~] = nLLfun(x0)
...
randseed = 108104778;
rng(randseed);
% a, b, noise, scale, six coherence levels
params = [0.00005	0.715294	46.838289	2981.030365	-0.001212	0.032153	0.08266	0.170666	0.280202	0.523662];
params = [	0.008446	0.804074	43.62477	2891.885696 [0 32 64 128 256 512]/1000];
w = ones(2);
...
initialvals = [Rstar,Rstar; (2-b(1,1))*Rstar,(2-b(2,2))*Rstar; b(1,1)*Rstar,b(2,2)*Rstar];
