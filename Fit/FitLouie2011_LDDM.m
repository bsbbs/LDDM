...
if visulize
    h = figure;  hold on;
    filename = sprintf('FitLouie_bads_%i_S%0.3e_G0-a%1.3f_B%2.1f_fval%1.3e_Rsqd%1.6f',i,S,G0_a,B,fval,Rsquared);
...
%%
function R1 = SolveEqlb(V,G0_a)
w = 1;
v = 1;
b = 0;
syms R1 R2 R3
if V(1) > 0 && V(2) > 0 && V(3) > 0
    eqns = [(V(1)/R1 - (w - b)*R1 - (1+G0_a) - v*R3)/v == R2, ... % dR1/dt = 0: R2 = (V(1)./R1 - (w - b)*R1 - (1-a) - v*R3)/v
        (V(2)/R2 - (w - b)*R2 - (1+G0_a) - v*R1)/v == R3, ... % dR2/dt = 0: R3 = (V(2)./R2 - (w - b)*R2 - (1-a) - v*R1)/v
        (V(3)/R3 - (w - b)*R3 - (1+G0_a) - v*R2)/v == R1]; % dR3/dt = 0: R1 = (V(3)./R3 - (w - b)*R3 - (1-a) - v*R2)/v;
    vars = [R1 R2 R3];
    [AnswR1, ~, ~] = solve(eqns, vars);
elseif V(1) > 0 && (V(2) <= 0 || V(3) <= 0)
    if V(2) <= 0 && V(3) > 0
        warning('V2 <= 0');
        eqns = [(V(1)/R1 - (w - b)*R1 - (1+G0_a) - v*R3)/v == 0, ... % dR1/dt = 0: R2 = (V(1)./R1 - (w - b)*R1 - (1-a) - v*R3)/v
            (V(3)/R3 - (w - b)*R3 - (1+G0_a))/v == R1]; % dR3/dt = 0: R1 = (V(3)./R3 - (w - b)*R3 - (1-a) - v*R2)/v;
        vars = [R1 R3];
        [AnswR1, ~] = solve(eqns, vars);
    elseif V(3) <= 0 && V(2) > 0
        warning('V3 <= 0');
        eqns = [(V(1)/R1 - (w - b)*R1 - (1+G0_a))/v == 0, ... % dR1/dt = 0: R2 = (V(1)./R1 - (w - b)*R1 - (1-a) - v*R3)/v
            (V(2)/R2 - (w - b)*R2 - (1+G0_a) - v*R1)/v == 0]; % dR2/dt = 0: R3 = (V(2)./R2 - (w - b)*R2 - (1-a) - v*R1)/v
        vars = [R1 R2];
        [AnswR1, ~] = solve(eqns, vars);
    elseif V(2) <= 0 && V(3) <= 0
        warning('V2, V3 <= 0');
