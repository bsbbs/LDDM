...
addpath('./SvrCode/');
out_dir = './Rslts/FitBhvr7ParamsIII_QMLE_SvrGPU';
...
UB = [70   2	100  20*256 [1,1,1]];
PLB = [15  .7	5    1*256 [.01 .01 .01]];
PUB = [60   1.2	40   8*256 [.2 .2 .2]];
...
% likelihood function
% parpool(6);
nLLfun = @(params) LcDFitBhvr7ParamsIII_QMLE_GPU(params, dataDynmc, dataBhvr);
[fvalbest,~,~] = nLLfun(x0)
...
dataDynmc = load('./Data/Data.mat');
dataBhvr = LoadRoitmanData('../../RoitmanDataCode');
randseed = 109340324;%109340324;
rng(randseed);
% a, b, noise, scale, tauR,G,I
params = [	21.369509	1.082589	24.404719	2520.316368	0.001617	0.346548	0.273613	16682.17228];

% simulation
tic;
[nLL, Chi2, BIC, AIC, rtmat, choicemat, argmaxR, sm_mr1c, sm_mr2c, sm_mr1cD, sm_mr2cD] = LcDFitBhvr7ParamsIII_QMLE_GPU(params,dataDynmc, dataBhvr);
name = sprintf('a%2.1f_b%1.1f_noise%2.1f_scl%2.1f_tau%1.3f_%1.3f_%1.3f_nLL%2.0f',params);
save(fullfile(plot_dir,sprintf('PlotData_%s.mat',name)),...
    'rtmat','choicemat','sm_mr1c','sm_mr2c','sm_mr1cD','sm_mr2cD','params','AIC','BIC','Chi2','nLL');
toc
