...
addpath('./SvrCode/');
out_dir = './Rslts/FitBhvr4ParamsIII_SvrGPU';
...
%     a,    b, noise, scale, sigma of Loglikelihood
LB = [0    0.1   .1    .1*256     0.0001];
UB = [70   2	100  20*256     10];
PLB = [15  .7	5    1*256      .001];
PUB = [60   1.2	40   8*256      .2];
...
% likelihood function
% parpool(6);
nLLfun = @(params) LcDFitBhvr4ParamsIII_GPU(params, dataDynmc, dataBhvr);
[fvalbest,~,~] = nLLfun(x0);
...
randseed = 108104778;
rng(randseed);
% a, b, noise, scale, sigmaLL
params = [0.008446	0.804074	43.62477	2891.885696	0.012127];
w = ones(2);
