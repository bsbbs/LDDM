...
for ti = -premotion_steps:max(total_time_steps(:))
    ...
    elseif all(size(onset_of_trigger) == size(V1mat))
    ...
    elseif all(size(onset_of_trigger) == size(V1mat))
        onset_of_trigger_Array = gpuArray(repmat(onset_of_trigger,1,1,sims));
        rt(flip) = ti - onset_of_trigger_Array(flip);
    end
    ...
    for excl = 1:numel(loc)
        ...
        elseif all(size(onset_of_trigger) == size(V1mat))
            excldt = rt(loc(excl)) + onset_of_trigger_Array(loc(excl)) - time_spc;
        end
