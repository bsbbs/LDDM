function nLL = LcDFitDynmcBhvrCut2_GPU(params,dataDynmc, dataBhvr)
...
Vinput = [V1, V2]*scale;
% tic;
[rtmat, choicemat, ~, sm_mr1c, sm_mr2c, sm_mr1cD, sm_mr2cD] = LcD_Dynmc_Trim_GPU(Vprior, Vinput, w, a, b,...
...
for i = 1:4
    ...
    for vi = 1:6
        if Nsse(vi) > 1
            ...
            ratio = Ndat(vi)/min(Nfit(vi),Ndat(vi));
            % 2. punish the numbders of fitted pionts odd with data
            punish = max(Nfit(vi),Ndat(vi))/min(Nfit(vi),Ndat(vi));
            Chi2 = Chi2 + sum(sse(~isnan(sse(:,vi)),vi))*ratio*punish;
        else % special case, simulation predicts weird no valid data
            Chi2 = Chi2 + sum(dat(~isnan(dat(:,vi)),vi).^2);
        end
    end
    N = N + sum(Ndat);
end
