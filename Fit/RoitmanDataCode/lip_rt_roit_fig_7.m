...
h.PaperUnits = 'inches';
h.PaperPosition = [0 0 5.2 4];
saveas(h,'DynmcsData.eps','epsc2');
saveas(h,'DynmcsData.fig','fig');
dot_ax = dot_ax';
sac_ax = sac_ax';
m_mr1c = nanrunmean(m_mr1c',1);
m_mr1cD = nanrunmean(m_mr1cD',1);
m_mr2c = nanrunmean(m_mr2c',1);
m_mr2cD = nanrunmean(m_mr2cD',1);
a = mean(m_mr1c(dot_ax >= 270 & dot_ax <= 310,:),1);
b = mean(m_mr2c(dot_ax >= 270 & dot_ax <= 310,:),1);
c = mean(m_mr1cD(sac_ax >= -70 & sac_ax <= -30,:),1);
d = mean(m_mr2cD(sac_ax >= -70 & sac_ax <= -30,:),1);

save('DynmcsData.mat','dot_ax','sac_ax','m_mr1c','m_mr1cD','m_mr2c','m_mr2cD','a','b','c','d');
