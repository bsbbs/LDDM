...
out_dir = fullfile(Glgdir,'Rslts/FitLouie2011/LDDMwBeta');
...
%         [x, fval, ~, report] = fmincon(f, x0, [],[],[],[], LB, UB, [], opt); % 
    ...
    save(fullfile(out_dir,'FitRslt.mat'),'summaries','xBest', 'S', 'BG_a','BR','beta','RSS_min','reports');
end
%% parameters recovery
% over G0_a
BG_a_rng = [-40, 40];
...
%%
function R1 = SolveEqlb(V,BG_a,b)
w = 1;
v = 1;
syms R1 R2 R3
...
for i = [find(PositiveRealAnsw)]'
    ...
    AnswD2 = b*AnswR2;
    AnswD3 = b*AnswR3;
    AnswG1 = w*AnswR1 + v*AnswR2  + v*AnswR3 - AnswD1;
    AnswG2 = v*AnswR1 + w*AnswR2  + v*AnswR3 - AnswD2;
    AnswG3 = v*AnswR1 + v*AnswR2  + w*AnswR3 - AnswD3;
